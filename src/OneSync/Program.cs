...
internal static class Program
{
    ...
    private static async Task<int> RunAsync(string[] args)
    {
        ...
        // Dev-time CLI probes (Phase D). These bypass normal sync init.
        for (int i = 0; i < args.Length; i++)
        {
            if (args[i].Equals("--cache-size-probe", StringComparison.OrdinalIgnoreCase) && i + 1 < args.Length)
            {
                ...
                var outputDir = Environment.ExpandEnvironmentVariables(
                    @"C:\Development\IAMCloud\LightweightCDM\docs\measurements");
                ...
                foreach (var drive in permitted)
                {
                    ...
                    try
                    {
                        ...
                        Action<string, System.IO.WatcherChangeTypes> onLocalChange = (path, change) =>
                        {
                            ...
                            // Pre-existing bug surfaced by Phase A Task 4 smoke test.
